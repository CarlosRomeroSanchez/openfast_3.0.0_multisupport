...
MODULE SD_FEM
  ...
  INTEGER(IntKi),   PARAMETER  :: PropSetsSLPMCol = 19                    ! Number of columns in SLPMPropSet (PropSetID, Kh,Kr,Kt,Ch,Cr,Ct,mh,Ir,mt,h1,h2,h3,Kz,Ktor,Mz,Itor,Cz,Ctor)
  ...
  INTEGER(IntKi),   PARAMETER  :: idMemberSLPM       = 4
...
SUBROUTINE SD_ReIndex_CreateNodesAndElems(Init,p, ErrStat, ErrMsg)
   ...
   DO iMem = 1, p%NMembers
      ...
      DO n=iMProp,iMProp+1
         ...
         else if (mType==idMemberSLPM) then
            sType='SLPM property'
            p%Elems(iMem,n) = FINDLOCI(Init%PropSetsSLPM(:,1), Init%Members(iMem, n) )   
         else
            ! Should not happen
...
SUBROUTINE SD_Discrt(Init,p, ErrStat, ErrMsg)
   ...
   INTEGER(IntKi)                :: nMemberCable, nMemberRigid, nMemberBeam, nMemberSLPM !< Number of memebers per type
   ...
   nMemberSLPM  = count(Init%Members(:,iMType) == idMemberSLPM) ! Count SLPM
   Init%NElem = nMemberBeam*Init%NDiv + nMemberCable + nMemberRigid + nMemberSLPM ! NOTE: only Beams are divided
   IF ( (nMemberBeam+nMemberRigid+nMemberCable+nMemberSLPM) /= size(Init%Members,1)) then
      CALL Fatal(' Member list contains an element which is not a beam, a cable, a rigid link or a SLPM'); return
   ENDIF
   ...
   ! --- Perform some sanity checks (irrespectively of NDiv) Would be better to do that before Reindexing...
    ...
    ! --- Cables and rigid link properties (these cannot be subdivided, so direct copy of inputs) AND SLPM
    Init%NPropC = Init%NPropSetsC
    Init%NPropR = Init%NPropSetsR
    Init%NPropSLPM = Init%NPropSetsSLPM
    ...
    CALL AllocAry(Init%PropsSLPM, Init%NPropSLPM, PropSetsSLPMCol, 'Init%PropsSLPM', ErrStat2, ErrMsg2); if(Failed()) return
    ...
    Init%PropsSLPM(1:Init%NPropSLPM, 1:PropSetsSLPMCol) = Init%PropSetsSLPM(1:Init%NPropSLPM, 1:PropSetsSLPMCol)
...
SUBROUTINE SetElementProperties(Init, p, ErrStat, ErrMsg)
   ...
   do I = 1, Init%NElem
      ...
      ! SLPM Init to excessive values to detect any issue
      p%ElemProps(i)%kh =  -9.99e+36
      p%ElemProps(i)%kr =  -9.99e+36
      p%ElemProps(i)%kt =   -9.99e+36
      p%ElemProps(i)%ch =   -9.99e+36
      p%ElemProps(i)%cr =   -9.99e+36
      p%ElemProps(i)%ct =   -9.99e+36
      p%ElemProps(i)%mh =   -9.99e+36
      p%ElemProps(i)%Ir =  -9.99e+36
      p%ElemProps(i)%mt =   -9.99e+36
      p%ElemProps(i)%h1 =   -9.99e+36
      p%ElemProps(i)%h2 =   -9.99e+36
      p%ElemProps(i)%h3 =   -9.99e+36
      p%ElemProps(i)%Kz =   -9.99e+36
      p%ElemProps(i)%Ktor = -9.99e+36
      p%ElemProps(i)%mz =   -9.99e+36
      p%ElemProps(i)%Itor = -9.99e+36
      p%ElemProps(i)%Cz =   -9.99e+36
      p%ElemProps(i)%Ctor =   -9.99e+36      
      ...
      !SLPM Properties
      else if (eType==idMemberSLPM) then
         if (DEV_VERSION) then
            print*,'Member',I,'is a SLPM'
         endif
         p%ElemProps(i)%kh =   Init%PropsSLPM(P1, 2) 
         p%ElemProps(i)%kr =   Init%PropsSLPM(P1, 3)
         p%ElemProps(i)%kt =   Init%PropsSLPM(P1, 4)
         p%ElemProps(i)%ch =   Init%PropsSLPM(P1, 5) 
         p%ElemProps(i)%cr =   Init%PropsSLPM(P1, 6)
         p%ElemProps(i)%ct =   Init%PropsSLPM(P1, 7)
         p%ElemProps(i)%mh =   Init%PropsSLPM(P1, 8)
         p%ElemProps(i)%Ir =   Init%PropsSLPM(P1, 9)
         p%ElemProps(i)%mt =   Init%PropsSLPM(P1, 10)
         p%ElemProps(i)%h1 =   Init%PropsSLPM(P1, 11)
         p%ElemProps(i)%h2 =   Init%PropsSLPM(P1, 12)
         p%ElemProps(i)%h3 =   Init%PropsSLPM(P1, 13)
         p%ElemProps(i)%Kz =   Init%PropsSLPM(P1, 14)
         p%ElemProps(i)%Ktor = Init%PropsSLPM(P1, 15)
         p%ElemProps(i)%mz =   Init%PropsSLPM(P1, 16)
         p%ElemProps(i)%Itor = Init%PropsSLPM(P1, 17)
         p%ElemProps(i)%Cz =   Init%PropsSLPM(P1, 18)
         p%ElemProps(i)%Ctor = Init%PropsSLPM(P1, 19)        
...
SUBROUTINE AssembleKM(Init, p, ErrStat, ErrMsg)
   ...
   INTEGER                  :: I, J, K
   INTEGER                  :: iGlob
   REAL(FEKi)               :: Ke(12,12),Ce_SLPM(12,12), Me(12, 12), FGe(12) ! element stiffness and mass matrices gravity force vector
   ...
   ! C_SLPM
   CALL AllocAry( Init%C_SLPM, p%nDOF, p%nDOF , 'Init%C_SLPM',  ErrStat2, ErrMsg2); if(Failed()) return; ! system SLPM matrix
   CALL AllocAry( p%FG,   p%nDOF,          'p%FG'  ,  ErrStat2, ErrMsg2); if(Failed()) return; ! system gravity force vector 
   Init%K  = 0.0_FEKi
   Init%C_SLPM  = 0.0_FEKi
   Init%M  = 0.0_FEKi
   p%FG    = 0.0_FEKi
   ...
   DO i = 1, Init%NElem
      ...
      CALL ElemC(p%ElemProps(i),Ce_SLPM)
      ...
      Init%C_SLPM(IDOF, IDOF) = Init%C_SLPM( IDOF, IDOF) + Ce_SLPM(1:12,1:12)
...
SUBROUTINE ElemM(ep, Me)
   ...
   if (ep%eType==idMemberBeam) then
      ...
      CALL ElemM_SLPM(ep%mh, ep%Ir, ep%mt, ep%h3, ep%mz, ep%Itor, Me)  
...
SUBROUTINE ElemK(ep, Ke)
   ...
   if (ep%eType==idMemberBeam) then
      ...
      CALL ElemK_SLPM(ep%kh, ep%kr, ep%kt, ep%h1, eP%Kz, eP%Ktor,Ke)   
...
SUBROUTINE ElemC(ep, Ce_SLPM)
   TYPE(ElemPropType), INTENT(IN) :: eP        !< Element Property
   REAL(FEKi), INTENT(OUT)        :: Ce_SLPM(12, 12)
   
   if (ep%eType==idMemberBeam) then
        Ce_SLPM = 0.0_FEKi
      
   else if (ep%eType==idMemberSLPM) then
      CALL ElemC_SLPM(ep%ch, ep%cr, ep%ct, ep%h2, ep%Cz, ep%Ctor, Ce_SLPM) 
