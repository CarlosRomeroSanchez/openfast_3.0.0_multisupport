...
SUBROUTINE SD_CalcOutput( t, u, p, x, xd, z, OtherState, y, m, ErrStat, ErrMsg )
      ...
      if ( p%nDOFM > 0) then
         ...
         m%UL_dotdot     = matmul( p%C2_61, x%qm    )    + matmul( p%C2_62   , x%qmdot )    & 
           ...
           IF (p%SLPMflag) THEN
             m%UL_dotdot  =  m%UL_dotdot + matmul( p%D2_62, m%udot_TP )
           endif !SLPMFLAG
      ...
      Y1_Utp  = - (matmul(p%KBB, m%u_TP) + matmul(p%CBB, m%udot_TP)  + matmul(p%MBB, m%udotdot_TP) )
            IF (p%SLPMflag) THEN
             Y1_Utp  = Y1_Utp - matmul(p%CBB_SLPM, m%udot_TP)
           endif !SLPMFLAG
      ...
      if (p%nDOFM>0) then
         ...
         !endif
         IF (p%SLPMflag) THEN
             Y1_Utp  = Y1_Utp + matmul(p%MBMCMB_SLPM,m%udot_TP )
         endif !SLPMFLAG
      endif
      if ( p%nDOFM > 0) then
...
SUBROUTINE SD_CalcContStateDeriv( t, u, p, x, xd, z, OtherState, m, dxdt, ErrStat, ErrMsg )
      ...
      IF (p%SLPMflag) THEN
           dxdt%qmdot = dxdt%qmdot - matmul(p%CMM_SLPM , x%qmdot) - matmul(p%CMB_SLPM , udot_TP)
      endif !SLPMFLAG
...
if (.not. LegacyFormat) then
   ...
   CALL AllocAry(Init%PropSetsR, Init%NPropSetsR, PropSetsRCol, 'RigidPropSets', ErrStat2, ErrMsg2); if(Failed()) return
    ...
    CALL ReadCom  ( UnIn, SDInputFile,                  'Simplified LPM-Tye Members Properties'          ,ErrStat2, ErrMsg2, UnEc ); if(Failed()) return
    CALL ReadIVar ( UnIn, SDInputFile, Init%NPropSetsSLPM, 'NPropSetsSLPM','Number of Simplified LPM properties',ErrStat2, ErrMsg2, UnEc ); if(Failed()) return
    p%SLPMflag = .false.
    if ( Init%NPropSetsSLPM> 0) then
        p%SLPMflag = .true.
    endif
    CALL ReadCom  ( UnIn, SDInputFile,                  'SLPM Properties Header'                          ,ErrStat2, ErrMsg2, UnEc ); if(Failed()) return
    CALL ReadCom  ( UnIn, SDInputFile,                  'SLPM Properties Unit  '                          ,ErrStat2, ErrMsg2, UnEc ); if(Failed()) return
    CALL AllocAry(Init%PropSetsSLPM, Init%NPropSetsSLPM, PropSetsSLPMCol, 'SLPMPropSets', ErrStat2, ErrMsg2); if(Failed()) return
    DO I = 1, Init%NPropSetsSLPM
       CALL ReadAry( UnIn, SDInputFile, Init%PropSetsSLPM(I,:), PropSetsSLPMCol, 'SLPMPropSets', 'SLPMPropSets ID and values ', ErrStat2, ErrMsg2, UnEc ); if(Failed()) return
    ENDDO   
    IF (Check( Init%NPropSetsSLPM < 0, 'NSLPMPropSets must be >=0')) return
...
SUBROUTINE SD_AM2( t, n, u, utimes, p, x, xd, z, OtherState, m, ErrStat, ErrMsg )
   ...
   ! note: matmul(F_L2,p%PhiM  ) = matmul(p%PhiM_T,F_L2) because F_L2 is 1-D
    ...
    IF (p%SLPMflag) THEN
         xq(1+p%nDOFM:2*p%nDOFM) = xq(1+p%nDOFM:2*p%nDOFM) + p%SDDeltaT *(- matmul(p%CMM_SLPM , x%qmdot) - matmul(p%CMB_SLPM , udot_TP2))
    endif !SLPMFLAG
...
SUBROUTINE SD_Craig_Bampton(Init, p, CB, ErrStat, ErrMsg)
   ...
   ! C_SLPM
   CALL AllocAry( CB%CBB_SLPM,    nR, nR,    'CB%MBB',    ErrStat2, ErrMsg2 ); CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   CALL AllocAry( CB%CBM_SLPM,    nR, nM,    'CB%MBM',    ErrStat2, ErrMsg2 ); CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName) 
   CALL AllocAry( CB%CMM_SLPM,    nM, nM,    'CB%MBM',    ErrStat2, ErrMsg2 ); CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)  
   ...
   CALL CraigBamptonReduction(Init, p, Init%M, Init%K, Init%C_SLPM, IDR, nR, p%ID__L, nL, nM, nM_out, CB%MBB, CB%MBM, CB%KBB,CB%CBB_SLPM, CB%CBM_SLPM, CB%CMM_SLPM, &
   CB%PhiL, CB%PhiR, CB%OmegaL, ErrStat2, ErrMsg2) 
   if(Failed()) return
   ...
   CALL SetParameters(Init, p, CB%MBB, CB%MBM, CB%KBB, CB%CBB_SLPM, CB%CBM_SLPM, CB%CMM_SLPM, PhiRb, nM_out, CB%OmegaL, CB%PhiL, ErrStat2, ErrMsg2)  
   CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,'Craig_Bampton')
...
contains
   ...
   subroutine applyConstr(CBParams, PhiRb)
      ...
      ! C_SLPM
      REAL(FEKi), ALLOCATABLE  :: CBBb_SLPM(:, :)
      REAL(FEKi), ALLOCATABLE  :: CBMb_SLPM(:, :)
      REAL(FEKi), ALLOCATABLE  :: CMMb_SLPM(:, :)
      ! "b" stands for "bar"
      ...
      ! C_SLPM
      CALL AllocAry( CBBb_SLPM,  p%nDOF__Rb, p%nDOF__Rb, 'matrix CBBb_SLPM',  ErrStat2, ErrMsg2 );
      CALL AllocAry( CBMb_SLPM,  p%nDOF__Rb, p%nDOFM,    'matrix CBmb_SLPM',  ErrStat2, ErrMsg2 );
      CALL AllocAry( CMMb_SLPM,  p%nDOFM, p%nDOFM,    'matrix CMMb_SLPM',  ErrStat2, ErrMsg2 );
      ...
      CBBb_SLPM  = CBparams%CBB_SLPM(p%nDOFR__-p%nDOFI__+1:p%nDOFR__, p%nDOFR__-p%nDOFI__+1:p%nDOFR__) 
      IF (p%nDOFM > 0) THEN   
         MBMb  = CBparams%MBM(p%nDOFR__-p%nDOFI__+1:p%nDOFR__, :               )
         CBMb_SLPM  = CBparams%CBM_SLPM(p%nDOFR__-p%nDOFI__+1:p%nDOFR__, :               )
         CMMb_SLPM = CBparams%CMM_SLPM(:,:)
      END IF
      PhiRb = CBparams%PhiR(              :, p%nDOFR__-p%nDOFI__+1:p%nDOFR__)
      deallocate(CBparams%MBB)
      deallocate(CBparams%KBB)
      deallocate(CBparams%MBM)
      deallocate(CBparams%CBB_SLPM)
      deallocate(CBparams%CBM_SLPM)  
       deallocate(CBparams%CMM_SLPM)   
      ...
      call move_alloc(CBBb_SLPM,  CBparams%CBB_SLPM)
      call move_alloc(CBMb_SLPM,  CBparams%CBM_SLPM)
      call move_alloc(CMMb_SLPM,  CBparams%CMM_SLPM)
...
SUBROUTINE SD_Guyan_RigidBodyMass(Init, p, MBB, ErrStat, ErrMsg)
   ...
   ! C_SLPM
   real(FEKi), allocatable :: CBB_SLPM(:, :)
   real(FEKi), allocatable :: CBM_SLPM(:, :)
   real(FEKi), allocatable :: CMM_SLPM(:, :)
   ...
   ! C_SLPM
   CALL AllocAry( CBB_SLPM,    nR, nR, 'MBB',    ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   CALL AllocAry( CBM_SLPM,    nR, nM, 'MBM',    ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   CALL AllocAry( CMM_SLPM,    nM, nM, 'MBB',    ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   ...
   CALL CraigBamptonReduction(Init, p, Init%M, Init%K, Init%C_SLPM, p%IDR__, nR, p%ID__L, nL, nM, nM_Out, MBB, MBM, KBB, CBB_SLPM, CBM_SLPM, CMM_SLPM, &
   PhiL, PhiR, OmegaL, ErrStat2, ErrMsg2)
   if(Failed()) return
...
SUBROUTINE SetParameters(Init, p, MBBb, MBmb, KBBb, CBBb_SLPM,CBMb_SLPM, CMMb_SLPM, PhiRb, nM_out, OmegaL, PhiL, ErrStat, ErrMsg)
   ...
   ! C_SLPM
   REAL(FEKi),               INTENT(IN   )   :: CBBb_SLPM(  p%nDOF__Rb, p%nDOF__Rb) ! Guyan mass matrix
   REAL(FEKi),               INTENT(IN   )   :: CBMb_SLPM(  p%nDOF__Rb, p%nDOFM)
   REAL(FEKi),               INTENT(IN   )   :: CMMb_SLPM(    p%nDOFM , p%nDOFM)
   ...
   ! C_SLPM
   p%CBB_SLPM = MATMUL( MATMUL( TI_transpose, CBBb_SLPM ), p%TI) != CBBt_SLPM
   ...
   IF ( p%nDOFM > 0 ) THEN ! These values don't exist for nDOFM=0; i.e., p%nDOFM == 0
      ...
      ! C_SLPM
      CALL LAPACK_gemm( 'T', 'N', 1.0_ReKi, p%TI, CBMb_SLPM, 0.0_ReKi, p%CBM_SLPM, ErrStat2, ErrMsg2) != CBMt
         CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName//'p%CBM_SLPM')
      p%CMM_SLPM = CMMb_SLPM  ! No transformation needed for MM 
      
      p%MMB = TRANSPOSE( p%MBM )                          != MMBt
      p%CMB_SLPM = TRANSPOSE( p%CBM_SLPM )                          != MMBt
      ...
      ! C_SLPM C1_12
     ! CALL LAPACK_GEMM( 'N', 'N', -1.0_ReKi, p%MBM,   p%CMM_SLPM,  1.0_ReKi, p%C1_12_SLPM, ErrStat2, ErrMsg2 )  ! p%C1_12_SLPM 
     ! CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName)
     IF (p%SLPMflag) THEN
      p%C1_12_SLPM = - MATMUL(p%MBM,p%CMM_SLPM)
      p%C1_12 = p%C1_12 + p%C1_12_SLPM + p%CBM_SLPM
     ENDIF ! ENDIF SLPM
      ...
      ! C_SLPM
      ! CALL LAPACK_GEMM( 'N', 'T', 1.0_ReKi, p%MBM,   p%CBM_SLPM,  0.0_ReKi, p%MBmCmB, ErrStat2, ErrMsg2 ); if(Failed()) return  ! MATMUL( p%MBM, p%MMB )
      CALL LAPACK_GEMM( 'N', 'T', 1.0_ReKi, p%MBM,   p%CBM_SLPM,  0.0_ReKi, p%MBMCMB_SLPM, ErrStat2, ErrMsg2 )  ! p%D1_12 
         CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName)
     ! p%D1_12 = p%CBB_SLPM - p%D1_12
      ...
      ! C_SLPM
      IF (p%SLPMflag) THEN
      p%C2_62 = p%C2_62  - MATMUL( p%PhiM , p%CMM_SLPM )
      ENDIF ! ENDIF SLPM
      ...
      !p%D2_64 = MATMUL( p%PhiM, p%CMB_SLPM)
      p%D2_62 = MATMUL( p%PhiM, p%CMB_SLPM ) ! ¿Negative?
      p%D2_62 = - p%D2_62
...
SUBROUTINE AllocParameters(p, nDOFM, ErrStat, ErrMsg)
   ...
   ! C_SLPM
   CALL AllocAry( p%CBB_SLPM,      nDOFL_TP, nDOFL_TP, 'p%CBB_SLPM',           ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters')
...
if (p%nDOFM > 0 ) THEN  
   ...
   ! C_SLPM
   CALL AllocAry( p%CBM_SLPM,           nDOFL_TP, nDOFM,    'p%CBM_SLPM',           ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters')
   CALL AllocAry( p%CMB_SLPM,           nDOFM,    nDOFL_TP, 'p%CMB_SLPM',           ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters')
   CALL AllocAry( p%CMM_SLPM,           nDOFM,    nDOFM, 'p%CMM_SLPM',           ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters')
   ...
   CALL AllocAry( p%C1_12_SLPM,   nDOFL_TP, nDOFM,    'p%C1_12_SLPM',         ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters')      
   ...
   CALL AllocAry( p%MBMCMB_SLPM,         nDOFL_TP, nDOFL_TP  , 'p%MBmCmB_SLPM',       ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters') ! is p%MBB when p%nDOFM == 0     
   ...
   ! C_SLPM
   CALL AllocAry( p%D2_62,         p%nDOF__L,  nDOFL_TP,'p%D2_64',       ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'AllocParameters') ! is zero when p%nDOFM == 0
end if
...
SUBROUTINE AllocMiscVars(p, Misc, ErrStat, ErrMsg)
   ...
   ! This shouldnt be necessary but, if not done here, there exists a problem at t=0.
      Misc%Ug = 0.0_ReKi
      Misc%Udotg = 0.0_ReKi
      Misc%Uddotg = 0.0_ReKi
