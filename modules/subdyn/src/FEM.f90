...
!! NOTE: generic code
SUBROUTINE BreakSysMtrx(MM, KK, CC_SLPM, IDR, IDL, nR, nL, MRR, MLL, MRL, KRR, KLL, KRL, CRR_SLPM, CLL_SLPM, CRL_SLPM, FG, FGR, FGL, CC, CRR, CLL, CRL)
   REAL(FEKi),             INTENT(IN   )  :: MM(:,:)   !< Mass Matrix
   REAL(FEKi),             INTENT(IN   )  :: KK(:,:)   !< Stiffness matrix
   REAL(FEKi),             INTENT(IN   )  :: CC_SLPM(:,:)   !< SLPM matrix
   ...
   ! C_SLPM
   REAL(FEKi),             INTENT(  OUT)  :: CRR_SLPM(nR, nR)
   REAL(FEKi),             INTENT(  OUT)  :: CLL_SLPM(nL, nL)
   REAL(FEKi),             INTENT(  OUT)  :: CRL_SLPM(nR, nL)   
...
!!
!! NOTE: generic code
SUBROUTINE CraigBamptonReduction(Init, p, MM, KK, CC_SLPM, IDR, nR, IDL, nL, nM, nM_Out, MBB, MBM, KBB, CBB_SLPM, CBM_SLPM, CMM_SLPM, PhiL, PhiR, OmegaL, ErrStat, ErrMsg, FG, FGR, FGL, FGB, FGM, CC, CBB, CBM, CMM) 
  ...
  ! INTEGER(IntKi),         INTENT(IN   ) :: nI        !< Number of Interface DOF   
   ...
   ! C_SLPM
   REAL(FEKi),             INTENT(  OUT) :: CBB_SLPM( nR, nR)     !< Reduced Guyan Mass Matrix
   REAL(FEKi),             INTENT(  OUT) :: CBM_SLPM( nR, nM)     !< Cross term
   REAL(FEKi),             INTENT(  OUT) :: CMM_SLPM( nM, nM)     !< Cross term
   ...
   ! C_SLPM
   real(FEKi), allocatable :: CRR_SLPM(:, :)
   real(FEKi), allocatable :: CLL_SLPM(:, :)
   real(FEKi), allocatable :: CRL_SLPM(:, :)
   ...
   ! C_SLPM
   CALL AllocAry(CRR_SLPM, nR, nR, 'matrix CRR_SLPM', ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)  
   CALL AllocAry(CLL_SLPM, nL, nL, 'matrix CLL_SLPM', ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)  
   CALL AllocAry(CRL_SLPM, nR, nL, 'matrix CRL_SLPM', ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   ...
   call BreakSysMtrx(MM, KK, CC_SLPM, IDR, IDL, nR, nL, MRR, MLL, MRL, KRR, KLL, KRL, CRR_SLPM, CLL_SLPM, CRL_SLPM, FG=FG, FGR=FGR, FGL=FGL, CC=CC, CRR=CRR, CLL=CLL, CRL=CRL)
   ! --- CB reduction
   call CraigBamptonReduction_FromPartition(Init, p, MRR, MLL, MRL, CRR_SLPM, CLL_SLPM, CRL_SLPM, KRR, KLL, KRL, nR, nL, nM, nM_Out,& !< Inputs 
                              MBB, MBM, KBB, CBB_SLPM, CBM_SLPM, CMM_SLPM, PhiL, PhiR, OmegaL, ErrStat2, ErrMsg2, & !< Outputs
...
contains
   ...
   subroutine CleanUp()
      ...
      ! C_SLPM
      IF(ALLOCATED(CRR_SLPM)  ) DEALLOCATE(CRR_SLPM) 
      IF(ALLOCATED(CLL_SLPM)  ) DEALLOCATE(CLL_SLPM) 
      IF(ALLOCATED(CRL_SLPM)  ) DEALLOCATE(CRL_SLPM)
...
!!
!! NOTE: generic code
SUBROUTINE CraigBamptonReduction_FromPartition(Init, p, MRR, MLL, MRL, CRR_SLPM, CLL_SLPM, CRL_SLPM, KRR, KLL, KRL, nR, nL, nM, nM_Out,&
                     MBB, MBM, KBB, CBB_SLPM, CBM_SLPM, CMM_SLPM, PhiL, PhiR, OmegaL, ErrStat, ErrMsg,&
  ...
  ! INTEGER(IntKi),         INTENT(  in)  :: nI   
   ...
   ! C_SLPM
   REAL(FEKi),             INTENT(  IN)  :: CRR_SLPM( nR, nR) !< Partitioned mass and stiffness matrices
   REAL(FEKi),             INTENT(  IN)  :: CLL_SLPM( nL, nL) 
   REAL(FEKi),             INTENT(  IN)  :: CRL_SLPM( nR, nL)
   ...
   ! C_SLPM
   REAL(FEKi),             INTENT(  OUT) :: CBB_SLPM( nR, nR)
   REAL(FEKi),             INTENT(  OUT) :: CBM_SLPM( nR, nM)
   REAL(FEKi),             INTENT(  OUT) :: CMM_SLPM( nM, nM)
   REAL(FEKi),             INTENT(  OUT) :: PhiR(nL, nR)     !< Guyan Modes   
   REAL(FEKi),             INTENT(  OUT) :: PhiL(nL, nM_Out) !< Craig-Bampton modes
   REAL(FEKi),             INTENT(  OUT) :: OmegaL(nM_Out)   !< Eigenvalues
   INTEGER(IntKi),         INTENT(  OUT) :: ErrStat          !< Error status of the operation
   ...
   REAL(FEKi) , allocatable :: PhiR_T_CLL(:,:)   ! PhiR_T_CLL(nR,nL) = transpose of PhiR * CLL (temporary storage)
   ! Seismic Parameters
   ...
   if (nL>0) then
      ...
      ! --- Set CBB_SLPM, CBM_SLPM, CMM_SLPM
      CALL AllocAry( PhiR_T_MLL,  nR, nL, 'PhiR_T_MLL', ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry( PhiR_T_CLL,  nR, nL, 'PhiR_T_MLL', ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry( Temp , nR, nR, 'Temp' , ErrStat2 , ErrMsg2); if(Failed()) return
         
      ! PhiR_T_MLL = TRANSPOSE(PhiR) * MLL AND PhiR_T_CLL
      CALL LAPACK_gemm( 'T', 'N', 1.0_FeKi, PhiR, MLL, 0.0_FeKi, PhiR_T_MLL  , ErrStat2, ErrMsg2); if(Failed()) return
      CALL LAPACK_gemm( 'T', 'N', 1.0_FeKi, PhiR, CLL_SLPM, 0.0_FeKi, PhiR_T_CLL  , ErrStat2, ErrMsg2); if(Failed()) return
      ! MBB1 = MATMUL(MRL, PhiR)
      CALL LAPACK_gemm( 'N', 'N', 1.0_FeKi, MRL, PhiR, 0.0_FeKi, MBB  , ErrStat2, ErrMsg2); if(Failed()) return
      CALL LAPACK_gemm( 'N', 'N', 1.0_FeKi, CRL_SLPM, PhiR, 0.0_FeKi, CBB_SLPM  , ErrStat2, ErrMsg2); if(Failed()) return
      ...
      CALL LAPACK_gemm( 'N', 'N', 1.0_FeKi, PhiR_T_CLL, PhiR, 0.0_FeKi, Temp  , ErrStat2, ErrMsg2); if(Failed()) return
      CBB_SLPM = CRR_SLPM + CBB_SLPM + TRANSPOSE( CBB_SLPM ) + Temp
      DEALLOCATE(Temp)
      ...
      ELSE
         ...
         CALL LAPACK_gemm( 'N', 'N', 1.0_FeKi, PhiR_T_CLL, PhiL(:,1:nM), 0.0_FeKi, CBM_SLPM  , ErrStat2, ErrMsg2); if(Failed()) return
         ...
         CALL LAPACK_gemm( 'N', 'N', 1.0_FeKi, CRL_SLPM, PhiL(:,1:nM), 0.0_FeKi, Temp  , ErrStat2, ErrMsg2); if(Failed()) return
         CBM_SLPM = Temp + CBM_SLPM    !This had PhiM   
         DEALLOCATE(Temp)
         ! CMM_SLPM
         CMM_SLPM = MATMUL( MATMUL( TRANSPOSE( PhiL(:,1:nM) ) , CLL_SLPM ) , PhiL(:,1:nM) ) 
      ENDIF
   ...
   END DO
     ...
     ! RM = PhiL_M(1:nM)
      ...
      IF (p%SLPMflag) THEN
      p%FISISC_U= CBB_SLPM(p%nDOFR__-p%nDOFI__+1:p%nDOFR__,1:p%nDOFR__-p%nDOFI__)
      ENDIF !SLPM
     ! 0.0_ReKi
     ...
     ! END IF
      ...
      IF (p%SLPMflag) THEN
      p%FMSISC_U=TRANSPOSE(CBM_SLPM(1:p%nDOFR__-p%nDOFI__,:))
      ENDIF ! SLPM
...
SUBROUTINE ElemK_SLPM(kh, kr, kt, h1, Kz, Ktor, K)
...
SUBROUTINE ElemM_SLPM(mh, Ir, mt, h3, mz, Itor, M)
...
END SUBROUTINE ElemM_SLPM

!------------------------------------------------------------------------------------------------------
!> Element damping matrix for simplified LPM members

SUBROUTINE ElemC_SLPM(ch, cr, ct, h2, Cz, Ctor, C)
